...
def thermal_latitude_profile(tm, lat, show=False, filename=None,
    ...
    if filename:
        ...
        fig.savefg(filename, transparent=True)#, dpi='figure', format='pdf')
    plt.close()
    return
...
def mechanic_latitude_profile(mm, lat, show=False, filename=None,
    ...
    heatmap = plt.pcolormesh(xx, zz, meca_masked.T, cmap=jet_white_r_2,
                             vmin=ys_min, vmax=ys_max) #shading='gouraud')
    plt.clim(ys_min, ys_max)
    cbar = plt.colorbar(heatmap)#, aspect='20')
    cbar.set_label('Yield Strength', rotation=90, labelpad=-50)
    plt.title('Mechanic Cross Section %.1f' %(lat))
    if show is True:
        plt.show()
    if filename:
        makedir_from_filename(filename)
        filename = filename + '_%.1f' %(lat) #+ '.png'
        fig.savefig(filename, transparent=True, format='ps')#, dpi='figure', format='pdf')
    plt.close()
    return
...
def elastic_thickness_latitude_profile(mm, lat, show=False, filename=None,
    ...
    plt.title('Perfil Espesor El?stico %.1f' %(lat))
    if show is True:
        plt.show()
    if filename:
        makedir_from_filename(filename)
...
#             title_1='Espesor El?stico Equivalente',
#             title_2='Espesor El?stico Efectivo',
...
#         xx, yy, array_2D_masked.T, shading='gouraud', alpha=alpha, vmin=0, vmax=150,
#         norm=norm, **kwargs)
...
def estimator_plot(
    ...
    if len(vnames) > 1:
        ...
        plt.plot(x_axis, estimator, 'k', linewidth=1.)
        # plt.scatter(x_axis, estimator, 'k')
